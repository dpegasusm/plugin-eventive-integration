		...
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Marquee Settings', 'eventive' ) }>
					<TextControl
						...
						help={ __( 'Filter by tag name (comma-separated)', 'eventive' ) }
					/>
					<TextControl
						label={ __( 'Exclude Tags', 'eventive' ) }
						value={ attributes.exclude }
						onChange={ ( value ) => setAttributes( { exclude: value } ) }
						help={ __( 'Exclude films with these tags', 'eventive' ) }
					/>
					<RangeControl
					...
					<ToggleControl
						...
						onChange={ ( value ) => setAttributes( { stills: value } ) }
					/>
					<ToggleControl
				...
				<PanelBody title={ __( 'Overlay & Caption', 'eventive' ) } initialOpen={ false }>
					<TextControl
						label={ __( 'Overlay Image URL', 'eventive' ) }
						value={ attributes.overlay }
						onChange={ ( value ) => setAttributes( { overlay: value } ) }
						help={ __( 'Background pattern/image URL', 'eventive' ) }
					/>
					<TextControl
					...
					<TextControl
						...
						onChange={ ( value ) => setAttributes( { captionSpeed: value } ) }
						help={ __( '"match" or seconds (e.g., "45")', 'eventive' ) }
					/>
				</PanelBody>
			</InspectorControls>
			...
			<div { ...blockProps }>
				<div className="eventive-block-placeholder">
					...
					<p>
						{ __( 'Film marquee will display on the frontend.', 'eventive' ) }
					</p>
					<p><strong>{ __( 'Settings:', 'eventive' ) }</strong></p>
					<ul>
						{ attributes.tag && <li>{ __( 'Tag:', 'eventive' ) } { attributes.tag }</li> }
						<li>{ __( 'Number:', 'eventive' ) } { attributes.number }</li>
						{ attributes.stills && <li>{ __( 'Using still images', 'eventive' ) }</li> }
						{ attributes.caption && <li>{ __( 'Caption:', 'eventive' ) } { attributes.caption }</li> }
					</ul>
				</div>
			</div>
		</>
