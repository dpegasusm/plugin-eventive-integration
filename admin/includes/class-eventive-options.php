...
class Eventive_Options {
    ...
    public function __construct() {
        ...
        add_action('admin_menu', [$this, 'eventive_admin_options_add_plugin_page']);
        add_action('admin_init', [$this, 'eventive_admin_options_page_init']);
    ...
    public function enqueue_admin_styles($hook_suffix) {
        if ($hook_suffix === 'toplevel_page_eventive-admin-options') {
            wp_enqueue_style(
                ...
                plugin_dir_url(dirname(__FILE__, 2)) . 'admin/css/eventive-admin.css',
                [],
                '1.0.0'
            );
        }
    ...
    public function eventive_admin_options_add_plugin_page() {
        add_menu_page(
            ...
            'manage_options',
            'eventive-admin-options',
            [$this, 'eventive_admin_options_create_admin_page'],
    ...
    if (!$api_key) {
        ...
        if (class_exists('Eventive_Sync_Films')) {
    ...
    public function eventive_admin_options_page_init() {
        register_setting(
            ...
            [$this, 'eventive_admin_options_sanitize']
        );

        add_settings_section(
            'eventive_admin_options_setting_section',
            'Settings',
            [$this, 'eventive_admin_options_section_info'],
    ...
    private function add_settings_fields() {
        add_settings_field(
            ...
            [$this, 'your_eventive_secret_key_2_callback'],
            'eventive-admin-options-admin',
            'eventive_admin_options_setting_section'
        );
        ...
        add_settings_field(
            ...
            [$this, 'your_eventive_event_bucket_1_callback'],
            'eventive-admin-options-admin',
            'eventive_admin_options_setting_section'
        );
        ...
        add_settings_field(
            'eventive_color_mode',
            'Color Mode',
            [$this, 'eventive_color_mode_callback'],
            'eventive-admin-options-admin',
            'eventive_admin_options_setting_section'
        );
    }
    ...
    public function eventive_admin_options_sanitize($input) {
        ...
        if (isset($input['eventive_color_mode'])) {
            $valid_modes = ['light', 'dark', 'default'];
    ...
    public function your_eventive_event_bucket_1_callback() {
        ...
        foreach ($buckets as $bucket) {
            ...
            echo sprintf('<option value="%s" %s>%s</option>', esc_attr($bucket['id']), $selected, esc_html($bucket['name']));
        }
        echo '</select>';
    }
