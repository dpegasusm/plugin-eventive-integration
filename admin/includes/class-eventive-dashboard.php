...
class Eventive_Dashboard {
    public function __construct() {
        ...
        add_action('wp_ajax_eventive_dashboard_data', [$this, 'eventive_dashboard_data']);
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    public function eventive_dashboard_data() {
        ...
        if (!$api_key || !$event_bucket_id) {
            ...
            wp_send_json_error(['message' => 'API Key or Event Bucket ID is missing'], 400);
            return;
        }
        ...
        $overview_response = wp_remote_get("https://api.eventive.org/charts/overview?event_bucket=$event_bucket_id", ['headers' => $headers]);
        if (is_wp_error($overview_response)) {
            error_log('Error fetching overview data: ' . $overview_response->get_error_message());
            wp_send_json_error(['message' => 'Failed to fetch overview data'], 500);
            return;
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE || empty($overview_data)) {
            ...
            wp_send_json_error(['message' => 'Invalid overview data'], 500);
            return;
        }
