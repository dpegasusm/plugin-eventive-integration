...
class Provincetown_Film {
	...
	public function init() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'provincetown_api_enqueue_scripts' ) );
		...
		add_action( 'updated_post_meta', array( $this, 'provincetown_update_agile_film_data' ), 10, 4 );
	}
	 ...
	 * @return void
	 */
	public function provincetown_api_enqueue_scripts() {
	 ...
	 */
	public function provincetown_update_agile_film_data( $meta_id, $object_id, $meta_key, $meta_value ) {
		if ( 'provincetown_film_agile_id' !== $meta_key ) {
			// This is not an agile film ID.
			return;
		}

		// Check that we have Metadata.
		if ( metadata_exists( 'post', $object_id, 'provincetown_film_agile_id' ) ) {
			// Get the Film scheule from Agile Ticketing.
			$agile_film_id = get_post_meta( $object_id, 'provincetown_film_agile_id', true );
		} else {
			return;
		}
		...
		if ( empty( $agile_film_id ) ) {
			update_post_meta( $object_id, 'provincetown_film_schedule', '' );
			return;
		}
		...
		switch ( $film_type ) {
			default:
			case 'films':
				$agile_url_base = esc_url_raw( maybe_unserialize( get_option( 'provincetown_festival_agile_url', '' ) ) );
				break;
			case 'event_films':
				$agile_url_base = esc_url_raw( maybe_unserialize( get_option( 'provincetown_special_agile_url', '' ) ) );
				break;
			case 'cinema_films':
				$agile_url_base = esc_url_raw( maybe_unserialize( get_option( 'provincetown_cinema_agile_url', '' ) ) );
				break;
		}

		// check if we already have a transient stored for this post ID.
		$film_info = get_transient( 'provincetown_film_agile_' . $agile_film_id );
		...
		if ( is_array( $film_json ) && ! is_wp_error( $film_json ) ) {
			...
			update_post_meta( $object_id, 'provincetown_film_schedule', maybe_serialize( $showings_list ) );
		}
	}
}
