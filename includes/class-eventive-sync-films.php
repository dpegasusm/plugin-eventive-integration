...
function eventive_build_film_excerpt( array $film, $limit = 180 ) {
	...
	$details  = isset( $film['details'] ) && is_array( $film['details'] ) ? $film['details'] : array();
	...
	$metaParts = array();
	if ( $director !== '' ) {
		$metaParts[] = "Director: $director";
	}
	if ( $runtime !== '' ) {
		$metaParts[] = "Runtime: $runtime min";
	}
	if ( $year !== '' ) {
		$metaParts[] = "Year: $year";
	}
	if ( $language !== '' ) {
		$metaParts[] = "Language: $language";
	}
...
class FilmSyncService {
	...
	public function get_eventive_api_credentials() {
		...
		return array( $event_bucket, $api_key );
	}

	public function fetch_eventive_films( $event_bucket, $api_key ) {
		$response = wp_remote_get(
			"https://api.eventive.org/event_buckets/$event_bucket/films",
			array(
		...
		$films = json_decode( $body, true )['films'] ?? array();
		return $films;
	}

	public function get_imported_pages() {
		$existing_film_pages = array();
		...
		foreach ( $film_query->posts as $post ) {
			...
			if ( $film_id ) {
				$existing_film_pages[ $film_id ] = array(
					'ID'     => $post->ID,
	...
	public function generate_imported_films_html( $imported_pages, $page = 1, $per_page = 15 ) {
		...
		foreach ( $paged_imports as $film ) {
			...
			// Categories and tags
			$categories = ( $post_type === 'post' ) ? wp_get_post_categories( $post_id, array( 'fields' => 'names' ) ) : array();
			$tags       = ( $post_type === 'post' ) ? wp_get_post_tags( $post_id, array( 'fields' => 'names' ) ) : array();
...
class Eventive_Sync_Films {
	...
	public function render_film_sync() {
		...
		if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['eventive_film_options_action'] ) && $_POST['eventive_film_options_action'] === 'save_options' ) {
			...
			$selected_categories = isset( $_POST['eventive_film_categories'] ) ? array_map( 'intval', (array) $_POST['eventive_film_categories'] ) : array();
			$selected_tags       = isset( $_POST['eventive_film_tags'] ) ? array_map( 'intval', (array) $_POST['eventive_film_tags'] ) : array();
		...
		$selected_categories = get_option( 'eventive_film_categories', array() );
		$selected_tags       = get_option( 'eventive_film_tags', array() );
...
function preview_eventive_films() {
	...
	if ( ! $event_bucket || ! $api_key ) {
		wp_send_json_error( array( 'message' => 'API credentials are missing.' ) );
		return;
	}
	...
	if ( is_wp_error( $films ) ) {
		wp_send_json_error( array( 'message' => $films->get_error_message() ) );
		return;
	}
	if ( empty( $films ) ) {
		wp_send_json_error( array( 'message' => 'No films found in the Eventive API response.' ) );
		return;
	}
	...
	$imported_films       = array_map(
		function ( $film ) {
			...
			return array(
				...
				'url'        => $film['url'],
				'post_type'  => $post_type,
				'categories' => ! empty( $categories ) ? array_values( $categories ) : array(),
	...
	$unsynced_films = array();
	foreach ( $films as $film ) {
		$film_id = $film['id'] ?? null;
		if ( ! $film_id || isset( $imported_pages[ $film_id ] ) ) {
			continue;
		}
		$film_name        = $film['name'] ?? '';
		$visibility       = $film['visibility'] ?? 'hidden';
...
function sync_eventive_films_with_wordpress() {
	...
	if ( ! is_array( $selected_film_ids ) ) {
		$selected_film_ids = array();
	}
	...
	if ( empty( $films['films'] ) ) {
		...
		error_log( $error_message );
		wp_send_json_error( array( 'message' => $error_message ), 404 );
		return;
	}
	...
	$post_type  = get_option( 'eventive_film_post_type', 'page' );
	$categories = get_option( 'eventive_film_categories', array() );
	$tags       = get_option( 'eventive_film_tags', array() );
	...
	foreach ( $films as $film ) {
		...
		try {
			if ( $existing_page_id ) {
				...
				if ( $post_type === 'post' ) {
					if ( ! empty( $categories ) ) {
						wp_set_post_categories( $existing_page_id, $categories );
					}
					if ( ! empty( $tags ) ) {
						wp_set_post_tags( $existing_page_id, $tags );
					}
				}
			} else {
				$new_page_id = wp_insert_post(
				...
				if ( $post_type === 'post' ) {
					if ( ! empty( $categories ) ) {
						wp_set_post_categories( $new_page_id, $categories );
					}
					if ( ! empty( $tags ) ) {
						wp_set_post_tags( $new_page_id, $tags );
					}
				}
...
add_action(
	'wp_ajax_eventive_toggle_post_status',
	function () {
		...
		if ( $result ) {
			wp_send_json_success( array( 'message' => "Post status changed to $new_status" ) );
		} else {
